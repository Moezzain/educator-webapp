...
const PatientSummaries = () => {
  ...
  const renderCard = () => {
   ...
   <CardContent style={styles.cardContent}>
     ...
     <Typography style={styles.learnMore} color="textSecondary" gutterBottom>
      <Button size="small" style={styles.learnMoreButton} onClick={() => (renderReadMore(note))}>{lang.ar.learnMore}</Button>
  ...
  return (
    ...
    >
      ...
      {currentPage === 'notes'? renderCard() :ReadMore(currentNote,renderNotes)  }
      </div>
    </div>
  );
};
